    ...
    // methodName is just the Analytics label ('google' | 'apple') — the two
    // providers otherwise flow through identical linkWithPopup/credential
    // recovery logic below.
    async function signInWithProvider(provider, methodName) {
        try {
            ...
            if (analytics) logEvent(analytics, isNewUser ? 'sign_up' : 'login', { method: methodName });
            return { user, isNewUser, suggestedName };
        } catch (err) {
            if (err.code === 'auth/credential-already-in-use' || err.code === 'auth/email-already-in-use') {
                // The account is already tied to a different (real) account.
                ...
                const credential = methodName === 'google'
                ...
                if (analytics) logEvent(analytics, 'login', { method: methodName });
    ...
    function updateScoreDisplay(animate = false) {
        ...
        if (!isPracticeMode) {
            ...
            if (score > currentHighScore) {
                if (animate) countUpDisplay(highScoreEl, currentHighScore, score);
                else highScoreEl.textContent = score;
...
// element so two words landing close together don't fight over the same
// digits each frame.
const activeCountUps = new WeakMap();
function countUpDisplay(el, from, to) {
    from = Math.round(from); to = Math.round(to);
    if (to <= from) { el.textContent = to; activeCountUps.delete(el); return; }
    const token = {};
    activeCountUps.set(el, token);
    const durationMs = Math.min(600, Math.max(150, (to - from) * 10));
    const start = performance.now();
    const tick = (now) => {
        if (activeCountUps.get(el) !== token) return; // superseded by a newer count-up
        const progress = Math.min(1, (now - start) / durationMs);
        el.textContent = from + Math.round((to - from) * progress);
...
function updateLeaderboardList(list, newEntry, sortKey, nestedKey = null) {
   ...
   function showWelcomeScreen() {
    ...
    function showAccountModal(initialTab = 'login', onSuccess = null) {
        ...
        if (isUserSignedIn()) {
            ...
            const appleSvg = `<svg class="w-4 h-4 mr-2 flex-shrink-0" viewBox="0 0 384 512" fill="currentColor"><path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z"/></svg>`;
            ...
            const renderAuthModal = (activeTab = 'login', errorMsg = '') => {
                ...
                if (activeTab === 'login') {
                    ...
                    accountModalContent.innerHTML = `
                        ...
                        <button id="login-apple-btn" class="w-full flex items-center justify-center bg-black hover:bg-slate-800 text-white font-semibold py-3 px-4 rounded-lg text-sm transition-colors mt-2">
                    ...
                    document.getElementById('login-google-btn').onclick = async () => {
                        ...
                        try {
                            const { isNewUser, suggestedName } = await signInWithProvider(new GoogleAuthProvider(), 'google');
                            if (isNewUser) {
                    ...
                    document.getElementById('login-apple-btn').onclick = async () => {
                        const btn = document.getElementById('login-apple-btn');
                        const btnHTML = btn.innerHTML;
                        btn.disabled = true;
                        btn.innerHTML = `<div class="flex items-center justify-center">${spinnerHtml}Signing in...</div>`;
                        try {
                            const { isNewUser, suggestedName } = await signInWithProvider(newAppleProvider(), 'apple');
                            if (isNewUser) {
                                renderNamePromptView(suggestedName);
                            } else {
                                finishAuth();
                    ...
                    accountModalContent.innerHTML = `
                        ...
                        <button id="signup-apple-btn" class="w-full flex items-center justify-center bg-black hover:bg-slate-800 text-white font-semibold py-3 px-4 rounded-lg text-sm transition-colors mt-2">
                            ${appleSvg}Continue with Apple
                        </button>
                    ...
                    document.getElementById('signup-apple-btn').onclick = async () => {
                        const btn = document.getElementById('signup-apple-btn');
                        const btnHTML = btn.innerHTML;
                        btn.disabled = true;
                        btn.innerHTML = `<div class="flex items-center justify-center">${spinnerHtml}Signing in...</div>`;
                        try {
   ...
   async function showLeaderboardModal(initialTab = 'challenge') {
    ...
    async function showProfileModal(defaultTab = 'profile') {
        ...
        const appleSvg = `<svg class="w-4 h-4 flex-shrink-0 text-black" viewBox="0 0 384 512" fill="currentColor"><path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z"/></svg>`;
        ...
        const renderModal = async (activeTab) => {
            ...
            let body;
            {
                ...
                const providerSection = isEmailUser ? `
                    ...
                    <div class="border-t border-slate-100 pt-4 flex items-center gap-2">
                        ${appleSvg}
                        <span class="text-xs text-slate-400">Signed in with Apple</span>
                    </div>` : !signedIn ? `
